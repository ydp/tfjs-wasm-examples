...
tf.setBackend('wasm').then(() => {
  ...
  async function run(epochCount, savePath) {
    ...
    await testValidationData.forEachAsync(data => {
      const evalOutput = model.evaluate(
          data.xs, data.ys, TEST_DATA_LENGTH);
    ...
    // }
    //await model.save(tf.io.http('http://127.0.0.1:5000/upload', {method: 'PUT'}));
    //await model.save(tf.io.browserHTTPRequest('http://127.0.0.1:5000/upload', {method: 'POST'}));
